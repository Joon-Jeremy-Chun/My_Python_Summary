...
+ 기본적으로 `숫자열`이면 __0을 제외한__ 모든 숫자를 인수로 하면 `True`가 나온다.
```python
...
+ 숫자뿐만 아니라 문자 더나아가 특수문자 까지도 순서를 정할 수있다. _`아스키 코드(American Standard Code for Information Interchange(ASCII))` 순_
+ 숫자 < 대문자 < 소문자 순서이다.
+ 첫문자가 같을경우 다음 문자를 비교한다.
```python
my_list = ["ab","Ac","ac","1","-10"]
min(my_list)
max(my_list)
```
